...
with TestClient(app) as c:
    ...
    # from a page somebody else wrote. It is not stored like an attachment: it
    # lives in the run's own directory and is deleted with the run, so the
    # checks below are about what the endpoint will resolve while it exists and
    # what it says once it does not.
    SHOT = b"\x89PNG\r\n\x1a\n" + os.urandom(2048)
    shot_dir = Path(browsing.make_run_dir(run_id))
    browsing.grants.issue(run_id, {"routes": [], "subnets": [], "systems": []}, None, "test", str(shot_dir))
    (shot_dir / "screenshot-001.png").write_bytes(SHOT)
    ...
    # not a name to sanitise — it is a request to refuse.
    for bad in [
        "screenshot-1.png",
        "screenshot-001.PNG",
    ...
    # A screenshot directory is writable by the agent — that is how it reads its
    # own captures back — so a link planted there with a name the endpoint does
    # accept is the way out that the name check alone would not close.
    try:
        (shot_dir / "screenshot-009.png").symlink_to("/etc/passwd")
        planted = True
    except (OSError, NotImplementedError):
        planted = False
    if planted:
        r = c.get(f"/api/runs/{run_id}/screenshots/screenshot-009.png")
        check("a symlink planted in the run's directory is refused",
    ...
    # Lifetime, which is the whole reason there is no second copy of this
    # anywhere: the turn ends, the directory goes, and the transcript keeps the
    # record without the pixels.
    browsing.grants.revoke(run_id)
    check("ending the run takes the screenshot off disk", not shot_dir.exists(), str(shot_dir))
    r = c.get(f"/api/runs/{run_id}/screenshots/screenshot-001.png")
    check("and the endpoint stops serving it", r.status_code == 404, str(r.status_code))
