...
def prepare(
    ...
    # The agent, not the app, is what runs ssh — so under accept-new it is the
    # agent that has to append the key it just learned. Left read-only the
    # connection still succeeds, but it warns and re-learns the host every time.
    os.chmod(known_hosts, RUN_SHARED_MODE)
