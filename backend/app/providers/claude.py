...
class ClaudeProvider(Provider):
    ...
    def build_run(
        ...
        *,
        ...
        return RunSpec(argv=argv, assigned_session_id=assigned)
    ...
    def parse_line(self, line: str) -> NormalizedEvent | None:
        ...
        if etype in ("assistant", "user"):
            ...
            if isinstance(blocks, str):
                return NormalizedEvent(kind=etype, text=blocks, raw=data)
            ...
            for block in blocks:
                ...
                if btype == "text":
                    ...
                    if thinking:
                        return NormalizedEvent(kind="thinking", text=thinking, raw=data)
                elif btype == "tool_use":
                    return NormalizedEvent(
                        kind="tool_use",
            ...
            if not joined:
                return None
            return NormalizedEvent(kind=etype, text=joined, raw=data)

        if etype == "result":
            is_error = bool(data.get("is_error")) or data.get("subtype") != "success"
            return NormalizedEvent(
                kind="result",
                text=data.get("result") or data.get("subtype"),
                raw=data,
                provider_session_id=data.get("session_id"),
